 ...
 * @license X-TRANS Develop License 1.0
 */
class Controller_Admin_Slider extends Controller_Admin_Base
{
    ...
    {
        $view      = View::forge('admin/slider/upload');
        $view->err = array();
        ...
        if (Input::method() == 'POST') {
            ...
            if (Upload::is_valid()) {
                ...
                $image->crop_resize(1680, 600)
                        //->watermark($pth.'wtm.png', "center middle")
                        ->save($filepath);

                $img               = Model_Slider::forge();
                $img->image_name   = $info['saved_as'];
                $img->upper_line   = Input::post('uline');
                $img->lower_line   = Input::post('dline');
        ...
        $this->template->title   = __('slider.upload');
        $this->template->content = $view;
    }
    ...
    {
        $img = Model_Slider::find($id);
        if (!$img) {
            Session::set_flash('error', __('message.img_not_exists'));
            Response::redirect('admin/slider');
        }

        $view      = View::forge('admin/slider/edit');
        $view->img = $img;
        $view->err = array();
        ...
        if (Input::method() == 'POST') {
            ...
            if (strlen($file['name']) > 0) {
                ...
                if (Upload::is_valid()) {
                    File::delete(DOCROOT . 'assets/img/slider/' . $img->image_name);
                    Upload::save();
                    $info     = Upload::get_files(0);
                    $filepath = $info['saved_to'] . $info['saved_as'];
                    $image    = Image::load($filepath);
    ...
    /**
     * Ajax: Change display_flag of Slider Image
     *
     * @param void
     *
     * @author Nguyen Van Hiep
