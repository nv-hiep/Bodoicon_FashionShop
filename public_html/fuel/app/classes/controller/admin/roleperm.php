 ...
 * @license X-TRANS Develop License 1.0
 */
class Controller_Admin_Roleperm extends Controller_Admin_Base
{
    ...
    public function action_register()
    {
        ...
        if (Input::method() == 'POST') {
            ...
            if ($val->run()) {
                if ($rp->perms_id == false) {
                    Session::set_flash('error', __('message.set_perm_first'));
                    Response::redirect('admin/roleperm/register');
                }
                if ($rp->save()) {
        ...
        $this->template->title   = __('button.add_roleperm');
        $this->template->content = $view;
    }
    ...
    public function action_ajaxload_actions()
    {
        ...
        foreach ($actions as $key => $action) {
            ...
            $ret .= "<div class='col-sm-6 checkbox'>
                     <label>
