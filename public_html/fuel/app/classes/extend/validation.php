...
class Validation extends Fuel\Core\Validation
{
    ...
    {
        Validation::active()->set_message('username', 'Tài khoản không hợp lệ');
        if (empty($val)) {
            return true;
        }
    ...
    {
        Validation::active()->set_message('phone_number', 'Điện thoại chỉ chấp nhận số');

        return (boolean) preg_match('/^\+*[0-9]*$/', $val);
    }
     ...
     * @author Dao Anh Minh
     */
    public static function _validation_unique_username($input_username, $edit_username = null)
    {
        Validation::active()->set_message('unique_username', 'Tên người dùng đã được sử dụng');

        $query = Model_Account::query()
                ->where('username', $input_username);

        if (!is_null($edit_username)) {
            $query->where('username', '!=', $edit_username);
        }
     ...
     * @author Dao Anh Minh
     */
    public static function _validation_quantity($val)
    {
        Validation::active()->set_message('quantity', 'Chỉ chấp nhận số từ 1->999');
        return (boolean) preg_match('/^[1-9][0-9]{0,2}$/', $val);
    }
}
